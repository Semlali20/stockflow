package com.stock.productservice.audit;
...
public class AuditAspect {
    ...
    private static final Map<String, String> RESOURCE_TYPE_MAP = Map.of(
        ...
        "ItemVariantController", "VARIANT",
        "ItemCsvController",     "PRODUCT"
    );
    ...
    private String resolveAction(String httpMethod, String methodName) {
        ...
        if ("POST".equals(httpMethod)) {
            if (m.contains("send") || m.contains("accept") || m.contains("reject") ||
                m.contains("validate") || m.contains("status") || m.contains("convert")) {
                return "STATUS_CHANGE";
            }
            if (m.contains("import") || m.contains("upload") || m.contains("csv")) return "IMPORT";
            return "CREATE";
        }
    ...
    private String extractResourceId(ProceedingJoinPoint pjp, Object result) {
        ...
        // Fallback: try getId() on the return value
        if (result != null) {
            try {
    ...
    private String buildDescription(String action, String type, String id) {
        ...
        return switch (action) {
            ...
            case "STATUS_CHANGE" -> "Status changed on " + type + suffix;
            case "IMPORT"        -> "Imported " + type + "s";
